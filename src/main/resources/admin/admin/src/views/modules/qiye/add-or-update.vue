	...
	<div class="addEdit-block" :style='{"padding":"20px 30px 30px","margin":"0 auto","color":"#666","borderRadius":"0 0 10px 10px","background":"#fff","width":"calc(100% - 60px)","fontSize":"16px"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"color":"inherit","margin":"0 0 20px 0","fontSize":"inherit"}' class="upload" v-if="type!='info' && !ro.touxiang" label="封面" prop="touxiang">
					<file-upload
						tip="点击上传封面"
						action="file/upload"
						:limit="3"
				...
				<el-form-item :style='{"color":"inherit","margin":"0 0 20px 0","fontSize":"inherit"}' class="upload" v-else-if="ruleForm.touxiang" label="封面" prop="touxiang">
				...
				<el-form-item :style='{"color":"inherit","margin":"0 0 20px 0","fontSize":"inherit"}' v-else-if="ruleForm.qiyejieshao" label="企业介绍" prop="qiyejieshao">
                    <span :style='{"fontSize":"16px","lineHeight":"40px","color":"inherit","fontWeight":"500","display":"inline-block"}' v-html="ruleForm.qiyejieshao"></span>
                </el-form-item>
...
export default {
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.qiyejieshao = this.ruleForm.qiyejieshao.replace(reg,'../../../springboottr95attj/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
