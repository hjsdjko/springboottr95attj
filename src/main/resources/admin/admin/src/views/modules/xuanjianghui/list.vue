		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"0px 10px 0","margin":"0px","fontSize":"inherit","overflow":"hidden","background":"none"}' :inline="true" :model="searchForm">
				<el-row :style='{"width":"100%","fontSize":"inherit","display":"block"}' >
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}' class="select" label="企业名称" prop="qiyemingcheng">
						...
						<el-select clearable v-model="searchForm.qiyemingcheng" placeholder="请选择企业名称" >
							<el-option v-for="(item,index) in qiyemingchengOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">在招职位</label>
						<el-input v-model="searchForm.zaizhaozhiwei" placeholder="在招职位" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">宣讲会地点</label>
						<el-input v-model="searchForm.xuanjianghuididian" placeholder="宣讲会地点" @keydown.enter.native="search()" clearable></el-input>
					</div>
				...
				<el-row class="actions" :style='{"width":"100%","margin":"10px 0 10PX","color":"rgba(19,161,230,1)","flexWrap":"wrap","display":"flex"}'>
					<el-button class="add" v-if="isAuth('xuanjianghui','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia13" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('xuanjianghui','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu7" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
						删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('xuanjianghui','审核')" :disabled="dataListSelections.length?false:true" type="success" @click="shBatchDialog()">
			...
			<div :style='{"border":"1px solid rgba(19,161,230,.2)","width":"100%","padding":"0","borderRadius":"0px","background":"#fff"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true'  
						prop="xuanjianghuididian"
						label="宣讲会地点">
						<template slot-scope="scope">
							{{scope.row.xuanjianghuididian}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
						prop="xuanjianghuishijian"
						label="宣讲会时间">
						<template slot-scope="scope">
							{{scope.row.xuanjianghuishijian}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
						prop="xinzifanwei"
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('xuanjianghui','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan14" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								更多
							</el-button>
							<el-button class="edit" v-if=" isAuth('xuanjianghui','修改')  && scope.row.sfsh=='待审核' " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('xuanjianghui','删除') " type="primary" @click="deleteHandler(scope.row.id )">
...
import AddOrUpdate from "./add-or-update";
    ...
    init () {
        this.sfshOptions = "是,否,待审核".split(',');
          this.$http({
            ...
            if (data && data.code === 0) {
              this.qiyemingchengOptions = data.data;
            } else {
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
           ...
           if(this.searchForm.zaizhaozhiwei!='' && this.searchForm.zaizhaozhiwei!=undefined){
            params['zaizhaozhiwei'] = '%' + this.searchForm.zaizhaozhiwei + '%'
          }
           if(this.searchForm.xuanjianghuididian!='' && this.searchForm.xuanjianghuididian!=undefined){
		...
		this.$refs["shBatchForm"].validate(valid => {
			if(valid){
				this.$confirm(`是否${this.batchIds.length>1?'一键审核':'审核'}选中数据?`, "提示", {
					...
					this.$http({
						url: "xuanjianghui/shBatch?sfsh="+this.shBatchForm.sfsh+"&shhf="+this.shBatchForm.shhf,
						method: "post",
						data: this.batchIds
					}).then(({ data }) => {
						if (data && data.code === 0) {
	...
	xuanjianghuistatusChange(e,row){
		if(row.status==0){
