					...
					<view id="waterfall-left-column-list-6" class="waterfall-column-list-6" :style='{"width":"50%","margin":"0 12rpx 0 0","flex":"1","flexDirection":"column","display":"flex"}'>
						<view @tap="onDetailTap(product)" :style='{"boxShadow":"none","margin":"0 0 20rpx 0","position":"relative"}' v-for="product in leftListList6" :key="product.id" class="left-content">
							...
							<view :style='{"width":"100%","padding":"8rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
								<view :style='{"padding":"16rpx 0","display":"flex"}' v-if="(userid && isAuth('yingpinxinxi','修改')) || (!userid && isAuthFront('yingpinxinxi','修改'))" @tap.stop.proevent="onUpdate" :data-row="product">
								...
								<view :style='{"padding":"16rpx 0","display":"flex"}' v-if="(userid && isAuth('yingpinxinxi','删除')) || (!userid && isAuthFront('yingpinxinxi','删除'))" @tap.stop.proevent="onDelete" :data-row="product">
							...
							<!-- #ifndef MP-WEIXIN -->
							<view :style='{"width":"100%","padding":"8rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
								<view :style='{"padding":"16rpx 0","display":"flex"}' v-if="(userid && isAuth('yingpinxinxi','修改')) || (!userid && isAuthFront('yingpinxinxi','修改'))" @tap.stop.proevent="onUpdateTap(product)">
									<text :style='{"margin":"0 10rpx 0 0","fontSize":"28rpx","lineHeight":"1","color":"#9e9e9e","display":"inline-block"}' class="cuIcon-edit"></text>
									<text :style='{"fontSize":"28rpx","lineHeight":"1","color":"#9e9e9e","display":"inline-block"}'>修改</text>
								</view>
								<view :style='{"padding":"16rpx 0","display":"flex"}' v-if="(userid && isAuth('yingpinxinxi','删除')) || (!userid && isAuthFront('yingpinxinxi','删除'))" @tap.stop.proevent="onDeleteTap(product.id)">
									<text :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"1","color":"#9e9e9e","display":"inline-block"}' class="cuIcon-delete"></text>
									<text :style='{"fontSize":"28rpx","lineHeight":"1","color":"#9e9e9e","display":"inline-block"}'>删除</text>
								</view>
							</view>
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`yingpinxinxi`, params);
                } else {
                    params['sfsh'] = '是';
                    res = await this.$api.list(`yingpinxinxi`, params);
                }
			...
			async search(){
                ...
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`yingpinxinxi`, searchForm);
                } else {
                    searchForm['sfsh'] = '是';
                    res = await this.$api.list(`yingpinxinxi`, searchForm);
                }
				// 如果是第一页数据置空
