		...
		<view :style='{"width":"100%","padding":"0","position":"relative","background":"#fff","height":"100%"}'>
			<view class="cu-bar bg-white search" :style='{"width":"100%","padding":"20rpx 24rpx","background":"#fff","display":"flex","height":"auto"}'>
				...
				<view :style='{"margin":"0 12rpx","flex":"1","position":"relative"}' v-if="queryIndex==0" class="search-form round">
					...
					<input :style='{"border":"2rpx solid #424EF0","padding":"12rpx 20rpx 12rpx 80rpx","color":"#000","borderRadius":"40rpx","background":"#fff","width":"100%","lineHeight":"56rpx","fontSize":"28rpx","height":"80rpx"}' v-model="searchForm.zhanghao" type="text" placeholder="账号" ></input>
				</view>
					...
					<view id="waterfall-left-column-list-6" class="waterfall-column-list-6" :style='{"width":"50%","margin":"0 12rpx 0 0","flex":"1","flexDirection":"column","display":"flex"}'>
						<view @tap="onDetailTap(product)" :style='{"boxShadow":"none","margin":"0 0 20rpx 0","position":"relative"}' v-for="product in leftListList6" :key="product.id" class="left-content">
							...
							<view :style='{"width":"100%","padding":"8rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
								<view :style='{"padding":"16rpx 0","display":"flex"}' v-if="(userid && isAuth('qiuzhizhe','修改')) || (!userid && isAuthFront('qiuzhizhe','修改'))" @tap.stop.proevent="onUpdate" :data-row="product">
								...
								<view :style='{"padding":"16rpx 0","display":"flex"}' v-if="(userid && isAuth('qiuzhizhe','删除')) || (!userid && isAuthFront('qiuzhizhe','删除'))" @tap.stop.proevent="onDelete" :data-row="product">
							...
							<!-- #ifndef MP-WEIXIN -->
							<view :style='{"width":"100%","padding":"8rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
								<view :style='{"padding":"16rpx 0","display":"flex"}' v-if="(userid && isAuth('qiuzhizhe','修改')) || (!userid && isAuthFront('qiuzhizhe','修改'))" @tap.stop.proevent="onUpdateTap(product)">
									<text :style='{"margin":"0 10rpx 0 0","fontSize":"28rpx","lineHeight":"1","color":"#9e9e9e","display":"inline-block"}' class="cuIcon-edit"></text>
									<text :style='{"fontSize":"28rpx","lineHeight":"1","color":"#9e9e9e","display":"inline-block"}'>修改</text>
								</view>
								<view :style='{"padding":"16rpx 0","display":"flex"}' v-if="(userid && isAuth('qiuzhizhe','删除')) || (!userid && isAuthFront('qiuzhizhe','删除'))" @tap.stop.proevent="onDeleteTap(product.id)">
									<text :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"1","color":"#9e9e9e","display":"inline-block"}' class="cuIcon-delete"></text>
									<text :style='{"fontSize":"28rpx","lineHeight":"1","color":"#9e9e9e","display":"inline-block"}'>删除</text>
								</view>
							</view>
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`qiuzhizhe`, params);
                } else {
                    res = await this.$api.list(`qiuzhizhe`, params);
                }
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('qiuzhizhe', JSON.stringify([id]));
			...
			async search(){
                ...
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`qiuzhizhe`, searchForm);
                } else {
                    res = await this.$api.list(`qiuzhizhe`, searchForm);
                }
				// 如果是第一页数据置空
