		...
		<view :style='{"width":"100%","padding":"0","position":"relative","background":"#fff","height":"100%"}'>
			<view class="cu-bar bg-white search" :style='{"width":"100%","padding":"20rpx 24rpx","background":"#fff","display":"flex","height":"auto"}'>
				...
				<view :style='{"margin":"0 12rpx","flex":"1","position":"relative"}' v-if="queryIndex==1" class="search-form round">
					<text class="icon iconfont icon-fangdajing07" :style='{"color":"rgb(153, 153, 153)","left":"0px","textAlign":"center","width":"80rpx","fontSize":"40rpx","lineHeight":"80rpx","position":"absolute","right":"0px"}'></text>
					<input :style='{"border":"2rpx solid #424EF0","padding":"12rpx 20rpx 12rpx 80rpx","color":"#000","borderRadius":"40rpx","background":"#fff","width":"100%","lineHeight":"56rpx","fontSize":"28rpx","height":"80rpx"}' v-model="searchForm.zaizhaozhiwei" type="text" placeholder="在招职位" ></input>
				</view>
				<view :style='{"margin":"0 12rpx","flex":"1","position":"relative"}' v-if="queryIndex==2" class="search-form round">
					<text class="icon iconfont icon-fangdajing07" :style='{"color":"rgb(153, 153, 153)","left":"0px","textAlign":"center","width":"80rpx","fontSize":"40rpx","lineHeight":"80rpx","position":"absolute","right":"0px"}'></text>
					<input :style='{"border":"2rpx solid #424EF0","padding":"12rpx 20rpx 12rpx 80rpx","color":"#000","borderRadius":"40rpx","background":"#fff","width":"100%","lineHeight":"56rpx","fontSize":"28rpx","height":"80rpx"}' v-model="searchForm.xuanjianghuididian" type="text" placeholder="宣讲会地点" ></input>
				</view>
				...
				<view id="list-box7" class="list-box7 waterfall-body" :style='{"width":"100%","padding":"20rpx 20rpx 0","alignItems":"flex-start","display":"flex","height":"auto"}'>
					<view id="waterfall-left-column-list-6" class="waterfall-column-list-6" :style='{"width":"50%","margin":"0 12rpx 0 0","flex":"1","flexDirection":"column","display":"flex"}'>
						<view @tap="onDetailTap(product)" :style='{"boxShadow":"none","margin":"0 0 20rpx 0","position":"relative"}' v-for="product in leftListList6" :key="product.id" class="left-content">
							<image :style='{"width":"100%","objectFit":"cover","borderRadius":"10rpx","display":"block","height":"300rpx"}' mode="widthFix" v-if="preHttp(product.fengmian)" :src="product.fengmian"></image>
							<image :style='{"width":"100%","objectFit":"cover","borderRadius":"10rpx","display":"block","height":"300rpx"}' mode="widthFix" v-else :src="product.fengmian?baseUrl+product.fengmian.split(',')[0]:''"></image>
							<view :style='{"width":"100%","flexWrap":"wrap","background":"none","display":"flex"}'>
								<view :style='{"padding":"0 20rpx","color":"#000","width":"100%","lineHeight":"2","fontSize":"28rpx","fontWeight":"bold","order":"1"}' class="title ">{{product.qiyemingcheng}}</view>
								<view :style='{"padding":"0 20rpx","color":"#000","width":"100%","lineHeight":"2","fontSize":"28rpx","fontWeight":"bold","order":"1"}' class="title ">{{product.zaizhaozhiwei}}</view>
							...
							<view :style='{"width":"100%","padding":"8rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
								<view :style='{"padding":"16rpx 0","display":"flex"}' v-if="(userid && isAuth('xuanjianghui','修改')) || (!userid && isAuthFront('xuanjianghui','修改'))" @tap.stop.proevent="onUpdate" :data-row="product">
								...
								<view :style='{"padding":"16rpx 0","display":"flex"}' v-if="(userid && isAuth('xuanjianghui','删除')) || (!userid && isAuthFront('xuanjianghui','删除'))" @tap.stop.proevent="onDelete" :data-row="product">
							...
							<!-- #ifndef MP-WEIXIN -->
							<view :style='{"width":"100%","padding":"8rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
								<view :style='{"padding":"16rpx 0","display":"flex"}' v-if="(userid && isAuth('xuanjianghui','修改')) || (!userid && isAuthFront('xuanjianghui','修改'))" @tap.stop.proevent="onUpdateTap(product)">
									<text :style='{"margin":"0 10rpx 0 0","fontSize":"28rpx","lineHeight":"1","color":"#9e9e9e","display":"inline-block"}' class="cuIcon-edit"></text>
									<text :style='{"fontSize":"28rpx","lineHeight":"1","color":"#9e9e9e","display":"inline-block"}'>修改</text>
								</view>
								<view :style='{"padding":"16rpx 0","display":"flex"}' v-if="(userid && isAuth('xuanjianghui','删除')) || (!userid && isAuthFront('xuanjianghui','删除'))" @tap.stop.proevent="onDeleteTap(product.id)">
									<text :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"1","color":"#9e9e9e","display":"inline-block"}' class="cuIcon-delete"></text>
									<text :style='{"fontSize":"28rpx","lineHeight":"1","color":"#9e9e9e","display":"inline-block"}'>删除</text>
								</view>
							</view>
	...
	export default {
		...
		methods: {
			...
			queryChange(e) {
				...
				this.searchForm.zaizhaozhiwei="";
				this.searchForm.xuanjianghuididian="";
			},
			//类别搜索
			...
			async upCallback(mescroll) {
				...
				if(this.searchForm.zaizhaozhiwei){
					params['zaizhaozhiwei'] = '%' + this.searchForm.zaizhaozhiwei + '%'
				}
				if(this.searchForm.xuanjianghuididian){
					params['xuanjianghuididian'] = '%' + this.searchForm.xuanjianghuididian + '%'
				}
				let user = uni.getStorageSync("appUserid")?JSON.parse(uni.getStorageSync('userSession')):{}
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`xuanjianghui`, params);
                } else {
                    params['sfsh'] = '是';
                    res = await this.$api.list(`xuanjianghui`, params);
                }
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('xuanjianghui', JSON.stringify([id]));
			...
			async search(){
				...
				if(this.searchForm.zaizhaozhiwei){
					searchForm['zaizhaozhiwei'] = '%' + this.searchForm.zaizhaozhiwei + '%'
				}
				if(this.searchForm.xuanjianghuididian){
					searchForm['xuanjianghuididian'] = '%' + this.searchForm.xuanjianghuididian + '%'
				}
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`xuanjianghui`, searchForm);
                } else {
                    searchForm['sfsh'] = '是';
                    res = await this.$api.list(`xuanjianghui`, searchForm);
                }
				// 如果是第一页数据置空
