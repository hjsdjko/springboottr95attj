	...
	<view :style='{"width":"100%","padding":"0","position":"relative","background":"#fff","height":"100%"}'>
		<form :style='{"width":"100%","padding":"24rpx","background":"#fff","display":"block","height":"auto"}' class="app-update-pv">
			...
			<view :style='{"padding":"12rpx 0","margin":"0 0 24rpx 0","borderColor":"#ccc","alignItems":"center","borderWidth":"0 0 2rpx 0","display":"flex","width":"100%","borderStyle":"solid","height":"auto"}' class=" select">
				...
				<picker :disabled="ro.qiyemingcheng" :style='{"width":"100%","flex":"1","height":"auto"}' @change="qiyemingchengChange" :value="qiyemingchengIndex" :range="qiyemingchengOptions">
					<view :style='{"width":"100%","lineHeight":"80rpx","fontSize":"26rpx","color":"#4753F2"}' class="uni-input">{{ruleForm.qiyemingcheng?ruleForm.qiyemingcheng:"请选择企业名称"}}</view>
				</picker>
			</view>
			<view :style='{"padding":"12rpx 0","margin":"0 0 24rpx 0","borderColor":"#ccc","alignItems":"center","borderWidth":"0 0 2rpx 0","display":"flex","width":"100%","borderStyle":"solid","height":"auto"}' class=" select">
...
<script>
	...
	export default {
		...
		async onLoad(options) {
			...
			// ss读取
			this.ruleForm.qiyemingcheng = this.user.qiyemingcheng
			...
			// 下拉框
			res = await this.$api.option(`qiye`,`qiyemingcheng`,{});
			this.qiyemingchengOptions = res.data;
            this.qiyemingchengOptions.unshift("请选择企业名称");
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`qiyegonggao`, this.ruleForm.id);
		...
		methods: {
			...
			// 下拉变化
			qiyemingchengChange(e) {
				this.qiyemingchengIndex = e.target.value
				this.ruleForm.qiyemingcheng = this.qiyemingchengOptions[this.qiyemingchengIndex]
			},
...
//跨表计算判断
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`qiyegonggao`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`qiyegonggao`, this.ruleForm);
						}else{
							await this.$api.add(`qiyegonggao`, this.ruleForm);
						}
